...
签到脚本只读取 `accessToken` 直接调接口，**本身没有任何续期逻辑**。所以 Secret 里那份副本迟早会失效，届时日志出现 `NO_SESSION`，签到随之停摆。

**不用自己记日子——每次运行都会告诉你还剩多久：**

```
账号 [me] 凭据就绪：uid=07b7****a3fb 昵称=... accessToken 到期=2026-11-09（剩 48 天）
```

剩余不足 3 天时还会额外抛一条 `::warning::` 提醒。看到天数不多了，或者日志里出现 `NO_SESSION`，就把**第 1 步和第 3 步重做一遍**：登录桌面端 → 复制文件内容 → 更新 Secret，两三分钟的事。

> [!NOTE]
> 有效期不是固定值——实测遇到过 30 天和 60 天两种（同一账号不同次登录也会不一样）。以日志里打印的那个到期日为准，别按天数倒推。

> [!NOTE]
> 桌面端会用本地凭据文件自行刷新 token，但**刷新的只是你电脑上那份文件，Secret 里的副本不会跟着变**。两边互相独立，所以仍然需要你手动同步一次。
...
| `NO_SESSION / HTTP 401\|403` | 凭据过期，重新登录桌面端并重新导出更新 Secret（见上文「凭据会过期」一节） |
